...
| distDir | String |  Quasar CLI 应该生成可分发文件的文件夹。项目根目录的相对路径。默认为 "dist/{ctx.modeName}"。适用于除 Cordova 以外的所有模式( Cordova 会被强制放到`src-cordova/www`)。|
| ignorePublicFolder | Boolean | 忽略/public文件夹。如果你依赖一个静态文件夹，那么你将需要自己配置它(在 Quasar 之外或通过extendWebpack/chainWebpack)，所以请确保你知道你在做什么。|
